...
def main():
    ...
    ap.add_argument("--out", required=True, help="output fp4 ONNX path")
    args = ap.parse_args()
    ...
    for name in tensor_input_names:
        ...
        print(f"[quantize_fp4] {name:22s} {feed[name].shape} {feed[name].dtype}")

    reader = _SingleSampleReader(feed)

    print(f"[quantize_fp4] quantizing {args.onnx} → {args.out} (NVFP4)")
    quantize(
        onnx_path=args.onnx,
        quantize_mode="fp4",
    ...
    print(f"[quantize_fp4] wrote {args.out}")
