...
def generate(args):
    ...
    if "t2v" in args.task:
        ...
        from trt_conv.export_stem import install_export_hook
        install_export_hook(wan_s2v.noise_model, onnx_path="/workspace/s2v_trt/stem.onnx")
        print(f"[diag] stem type: {type(wan_s2v.noise_model.stem).__name__}")
        print(f"[diag] stem forward id after install: {id(wan_s2v.noise_model.stem.forward)}")
        print(f"[diag] stem forward qualname: {getattr(wan_s2v.noise_model.stem.forward, '__qualname__', '?')}")
